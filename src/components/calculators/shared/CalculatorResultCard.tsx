...
const CalculatorResultCard: React.FC<CalculatorResultCardProps> = ({ metrics, chart, title, subtitle, explanations, delay = 0 }) => (
    <motion.div
        ...
        className="w-full max-w-xl mx-auto h-full bg-gradient-to-br from-[#F5F1E8]/90 to-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-[#E6C674]/30 overflow-hidden"
    >
            ...
            <div className="pb-4 bg-gradient-to-r from-[#F5F1E8] to-[#E6C674]/20 px-8 pt-6">
                <div className="flex items-center gap-3">
                    <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-[#F5E7D6]">
                        <span className="text-2xl text-[#B4A078]"><FaChartBar /></span>
                    </span>
                    <div>
        ...
        <div className="p-8 space-y-6">
            ...
            {explanations && explanations.length > 0 && (
                <div className="mt-6 bg-[#f9f5ef] rounded-xl p-4 shadow text-left max-w-md w-full mx-auto">
                    {explanations.map((exp, idx) => (
                        <div className="mb-2" key={exp.label}>
                            ...
                            <span className="ml-2 text-tertiary/80">{exp.description}</span>
                        </div>
                    ))}
                </div>
            )}
